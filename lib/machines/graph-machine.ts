...
import type { GraphContext, GraphEvent, GraphSnapshot, GraphStats, GraphNode, GraphEdge } from "@/lib/types";
...
export function createGraphMachine(algorithmId: string, initialData?: GraphInitialData) {
  ...
  const regenerate = (ctx: GraphContext) => {
  ...
  const playReset = (ctx: GraphContext) => {
    const g = generateRandomGraph(ctx.nodeCount);
    return {
      nodes: g.nodes,
      edges: g.edges,
